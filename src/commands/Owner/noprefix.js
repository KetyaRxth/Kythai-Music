...
// Owner IDs
const ownerIDS = [
  '1383706658315960330',// Danav's Id
];

// Support server restriction
const SUPPORT_SERVER_ID = "1394222392879874128";

// Webhook for logs (replace with your own)
const logWebhook = new WebhookClient({ 
  url: "https://discord.com/api/webhooks/1425836183417524304/fhkUFsXZGx31jfL4qK48bsoYFOHRM5unC99nys09AXectB3t5DWUtZ2ch3cxIwMYCFmX" 
});
...
module.exports = {
  ...
  run: async (client, message, args, prefix) => {
    // Support server check
    if (message.guild?.id !== SUPPORT_SERVER_ID) {
      return message.reply({ 
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription("This command can only be used inside the **Support Server**.")
        ]
      });
    }
    ...
    try {
      ...
      if (args[0].toLowerCase() === "add") {
        if (!userId) {
          return message.channel.send({
            embeds: [new EmbedBuilder().setColor("Red").setDescription("❌ Please mention a user or provide a valid ID to add.")]
          });
        }
      ...
      if (args[0].toLowerCase() === "remove") {
        if (!userId) {
          return message.channel.send({
            embeds: [new EmbedBuilder().setColor("Red").setDescription("❌ Please mention a user or provide a valid ID to remove.")]
          });
        }
        ...
        if (!data) {
          return message.channel.send({
            embeds: [new EmbedBuilder().setColor("Orange").setDescription("⚠️ This user does not have NoPrefix.")]
          });
        }
      ...
      if (["list", "show"].includes(args[0].toLowerCase())) {
        ...
        if (!data.length) {
          return message.channel.send({
            embeds: [new EmbedBuilder().setColor("Grey").setDescription("ℹ️ No users currently have NoPrefix.")]
          });
        }
...
async function sendLog(action, executor, targetId, duration = null) {
  const logEmbed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "🛠️ Executor", value: `${executor.tag} (\`${executor.id}\`)`, inline: false },
